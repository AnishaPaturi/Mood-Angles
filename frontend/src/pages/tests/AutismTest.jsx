...
const { questions, answers, handleSelect, attempt } = useDynamicQuestions("autism", defaultQuestions);
  ...
  useEffect(() => {
    const fetchPreviousResults = async () => {
      if (!userId) return;
      try {
        const res = await fetch(`${API_BASE}/api/results/previous/autism?userId=${userId}`);
        if (res.ok) {
