...
import { Menu, X, FileText, Upload, MessageCircle, HeartHandshake, Info } from "lucide-react";
...
function Dashboard() {
  ...
  const styles = `
    ...
    *{box-sizing:border-box;font-family:Inter, system-ui, -apple-system, 'Segoe UI', Roboto, 'Helvetica Neue', Arial}
    ...
    .header{display:flex;align-items:center;justify-content:space-between;padding:18px 22px;background:var(--card);backdrop-filter:blur(6px);box-shadow:0 4px 18px rgba(20,20,40,0.06);transition:all 300ms ease}
    ...
    .avatar{width:40px;height:40px;border-radius:50%;background:linear-gradient(180deg,#b894ff,#9b6bff)}
  ...
  const todayIndex = new Date().getDay() === 0 ? 6 : new Date().getDay() - 1; // Sunday fix
      ...
      <div className="layout">
        ...
        <div className={`main ${sidebarOpen ? '' : 'expanded'}`}>
          ...
          <div className="content">
            {/* Dynamic Quote in the center with animation */}
            <div key={quoteIndex} className="quoteCenter">“{quotes[quoteIndex]}”</div>

            <div className="grid3">
              <FeatureCard title="Take a Test" icon={<FileText />} desc="Assess your mood with a quick test." />
              <FeatureCard title="Upload Documents" icon={<Upload />} desc="Keep your important files in one place." />
              <FeatureCard title="Talk to Someone" icon={<MessageCircle />} desc="Connect with a counselor instantly." />
            </div>

            <div className="grid2">
              <FeatureCard title="Community Support" icon={<HeartHandshake />} desc="Join group sessions and support circles." />
              <FeatureCard title="Resources & Tips" icon={<Info />} desc="Read articles and guides for better mental health." />
            </div>

            <div className="progressWrap">
              <h4 style={{margin:0,marginBottom:8,color:'#3b3b4a'}}>Your Weekly Progress</h4>
              <div className="progressBar"><div className="progressFill" style={{ width: progress + '%' }} /></div>
              <div className="progressMeta">{streak > 0 ? `You’ve logged in ${streak} days in a row ✨` : 'Start your streak today!'} (Progress: {Math.round(progress)}%)</div>

              <div className="streakCalendar">
                {daysOfWeek.map((day, idx) => (
                  <div key={idx} className={`dayCircle ${idx <= todayIndex && streak > idx ? 'active' : ''}`}>{day[0]}</div>
                ))}
              </div>
            </div>
          </div>

          <div className="footer">© {new Date().getFullYear()} MoodAngels • Crafted with 💜 to support your well-being</div>
        </div>
      </div>
    </div>
  );
